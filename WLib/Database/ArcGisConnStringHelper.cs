...
namespace WLib.Database
{
    ...
    /// （需要安装ESRI OLE DB提供程序（随安装ArcGIS Desktop提供），注意事项请参考本类所在文件的注释部分）
    /// （参考：http://resources.esri.com/help/9.3/ArcGISEngine/dotnet/0ec7f577-5dbd-4a60-b1f3-d5ef4a1426e4.htm）
    /// </summary>
    public static class ArcGisConnStringHelper
    {
        /// <summary>
        ...
        public static string ShpMdbGdb(string sourcePath, string geometry = "WKB")
        {
            if (File.Exists(sourcePath))
            {
                ...
                if (extension == ".shp") return Shapefile(sourcePath, geometry);
                if (extension == ".dbf") return Dbf(sourcePath);
                if (extension == ".mdb") return Access(sourcePath, geometry);
            }
            if (Directory.Exists(sourcePath) && sourcePath.EndsWith(".gdb"))
                return FileGdb(sourcePath);
        ...
        /// <param name="geometry">值为"WKB"和"OBJECT"之一</param>
        /// <returns></returns>
        public static string Access(string mdbPath, string geometry = "WKB")
        {
        ...
        /// <returns></returns>
        public static string DbfDirectory(string dbfDirectory)
        {
            return $"Provider=MICROSOFT.JET.OLEDB.4.0;Data Source={dbfDirectory};Extended Properties=dBase IV;";
        }
        /// <summary>
