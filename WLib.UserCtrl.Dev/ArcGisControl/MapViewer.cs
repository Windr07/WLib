...
namespace WLib.UserCtrls.Dev.ArcGisControl
{
    ...
    public partial class MapViewer : UserControl, IViewer
    {
        ...
        public MapViewer()
        {
            ...
            LayerRemoved = new EventHandler((obj, evt) =>
            {
                if (_attributeForm != null && !_attributeForm.IsDisposed)
                {
                    if (_attributeForm.Layer != null)
                    {
            ...
            MapActiveViewEvents.ItemAdded += new IActiveViewEvents_ItemAddedEventHandler((item) =>
            {
                if (item is ILayer)
                {
                    if (LayerAdded != null)
                        LayerAdded(item as ILayer, new EventArgs());
                    GoToMapView();
                    GotoLayerTOC();
                }
                else if (item is ITable)
                {
                    AddTable(item as ITable);
                    GoToMapView();
                    GotoTableListBox();
                }
            });
        ...
        {
            GoToMapView();
            ZoomTo.MapZoomToAndSelectFirst(axMapControlMainMap, featureLayer, whereClause);
        }
        /// <summary>
        /// 显示指定图层指定字段的标注
        /// </summary>
        ...
        public void UpDateEagleMap()
        {
            ...
            //this.axMapControlEagleMap.Extent = this.axMapControlMainMap.FullExtent;//设置鹰眼显示范围为全局范围
            #endregion

            #region 主地图更换地图后，布局控件的地图随之更换
            //
            //TODO:主地图更换地图后，布局控件的地图随之更换
            //
            copyMapToPageLayout();
            #endregion
        }
        ...
        public void LoadDbFile(string strConOrPath)
        {
            ...
            var featureClasses = FindFeatureClass.GetFeatureClasses(workspace);
            featureClasses = FeatClassOpt.SortByGeometryType(featureClasses);
        ...
        public void AddTable(ITable table)
        {
            ...
            if (table as IObjectClass != null)
                tableName = (table as IObjectClass).AliasName;
            else
                tableName = (table as IDataset).Name;
        ...
        public void AddTables(IEnumerable<ITable> tables)
        {
            ...
            var tableNamesItems = tables.Select(v => (v as IObjectClass).AliasName).Select(v => new ImageListBoxItem(v, 17)).ToArray();
                ...
                switch (statusFlag)
                {
                    case 8: //测距离
                        ...
                        else
                        {
                            if (_measureTool.IsSurveying)
                            {
                                object lineSymbolObj;
                                lineSymbolObj = new SimpleLineSymbolClass();
                                lineSymbolObj = RenderOpt.getSimpleLineSymbol("ff0000");
                                #region 设置样式
                                //ISimpleLineSymbol isl = lineSymbolObj as ISimpleLineSymbol;
                                //isl.Color = CommonLib.getIColor("00ff00");
                                //isl.Style = esriSimpleLineStyle.esriSLSDashDot;
                                #endregion
                    ...
                    case 9: //测面积
                        ...
                        else
                        {
                            ...
                            {
                                object fillSymbolObj;
                                fillSymbolObj = new SimpleFillSymbolClass();
                                fillSymbolObj = RenderOpt.GetSimpleFillSymbol("99ccff", "ff0000");
        ...
        private void ShowFieldLabelMenuItems(ILayer setFieldLabelLayer, string fontName = "宋体", int size = 12)//将图层的字段名作为菜单显示，用于设置标注
        {
            ...
            {
                var namesDict = FieldOpt.GetFieldNameAndAliasName(table);
                ...
                foreach (var namePair in namesDict)
                {
                    ...
                    item.Click += (sender2, e2) =>
                    {
                        ...
                        if (geoFeatureLayer != null)
                        {
                            if (item.Checked)
                            {
                                LabelAnnoOpt.ShowLabel(geoFeatureLayer, namePair.Key.ToString(), fontName, size);
