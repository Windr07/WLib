...
namespace WLib.UserCtrls.Dev.ArcGisCtrl
{
    ...
    {
        private IStyleGalleryItem _pStyleGalleryItem;//图形目录、图库
        private ILegendClass _pLegendClass;//图例类
        private ILayer _pLayer;
        ...
        private void PreviewImage()
        {
            stdole.IPictureDisp picture = this.axSymbologyControl1.
                ...
                PreviewItem(_pStyleGalleryItem, this.ptbPreview.Width, this.ptbPreview.Height);
        ...
        {
            ((IMarkerSymbol)this._pStyleGalleryItem.Item).Size =
        ...
        private void nudWidth_ValueChanged(object sender, EventArgs e)
        {
            ...
            {
                case esriSymbologyStyleClass.esriStyleClassLineSymbols:
                    ((ILineSymbol)this._pStyleGalleryItem.Item).Width = Convert.ToDouble(this.nudWidth.Value);
                    break;
                case esriSymbologyStyleClass.esriStyleClassFillSymbols:
                    //取得面符号的轮廓线符号
                    ILineSymbol pLineSymbol = ((IFillSymbol)this._pStyleGalleryItem.Item).Outline;
                    pLineSymbol.Width = Convert.ToDouble(this.nudWidth.Value);
                    ((IFillSymbol)this._pStyleGalleryItem.Item).Outline = pLineSymbol;
                    break;
            }
            this.PreviewImage();
        }
        ...
        {
            ((IMarkerSymbol)this._pStyleGalleryItem.Item).Angle = (double)this.nudAngle.Value;
            this.PreviewImage();
        }
        ...
        private void btnColor_EditValueChanged(object sender, EventArgs e)
        {
            ...
            switch (this.axSymbologyControl1.StyleClass)
            {
                //点符号
                case esriSymbologyStyleClass.esriStyleClassMarkerSymbols:
                    ((IMarkerSymbol)this._pStyleGalleryItem.Item).Color = color;
                    break;
                //线符号
                case esriSymbologyStyleClass.esriStyleClassLineSymbols:
                    ((ILineSymbol)this._pStyleGalleryItem.Item).Color = color;
                    break;
                //面符号
                case esriSymbologyStyleClass.esriStyleClassFillSymbols:
                    ((IFillSymbol)this._pStyleGalleryItem.Item).Color = color;
                    break;
            }
            //更新符号预览
            this.PreviewImage();
        ...
        {
            //取得选定的符号
            this.PSymbol = (ISymbol)_pStyleGalleryItem.Item;
            //更新预览图像
