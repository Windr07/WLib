...
namespace WLib.ArcGis.GeoDatabase.FeatClass
{
    ...
    public static class FeatClassToPath
    {
        ...
        public static IFeatureClass CreateToPath(string fullPath, IFields fields)
        {
            ...
            else if (fullPath.Contains(".mdb"))
            {
                ...
                if (!File.Exists(dbPath))
                    WorkspaceCreate.NewWorkspace(EWorkspaceType.Access, Path.GetDirectoryName(dbPath), Path.GetFileNameWithoutExtension(dbPath));
            ...
            else if (fullPath.Contains(".gdb"))
            {
                ...
                if (!Directory.Exists(dbPath))
                {
                    ...
                    WorkspaceCreate.NewWorkspace(EWorkspaceType.FileGDB, dirInfo.Parent.FullName, dirInfo.Name);
                }
                var names = fullPath.Replace(dbPath, "").Split(new[] { Path.DirectorySeparatorChar, Path.AltDirectorySeparatorChar }, StringSplitOptions.RemoveEmptyEntries);
                if (names.Length == 1) return CreateToDb(dbPath, null, names[0], fields);
                if (names.Length == 2) return CreateToDb(dbPath, names[0], names[1], fields);
        ...
        {
            return CreateToDb(Path.GetDirectoryName(shpPath), Path.GetFileNameWithoutExtension(shpPath), null, geoType, spatialRef, otherFields);
        }
        /// <summary>
        /// 新建一个Shp文件，并返回shp文件存储的要素类（注意目录中不能存在同名文件）
        /// </summary>
        ...
        {
            return CreateToDb(Path.GetDirectoryName(shpPath), Path.GetFileNameWithoutExtension(shpPath), null, fields);
        }
        ...
        {
            var workspace = GetWorkspace.GetWorkSpace(geoDbPath);
            var spatialRef = fields.GetSpatialReference();
            IFeatureClass featureClass;
            if (!string.IsNullOrEmpty(datasetName))
            {
                var featureDataset = workspace.GetFeatureDataset(datasetName) ?? workspace.CreateFeatureDataset(datasetName, spatialRef);
