...
namespace WLib.ArcGis.Analysis.Gp
{
    ...
    public partial class GpHelper
    {
        ...
        {
            return PathEx.FileNameIsValid(path) && PathEx.FolderNameIsValid(path) ?
                null : "路径为空或包含特殊字符：\\/:*?\"&<>|";
        }
        /// <summary>
        /// 检查路径是否超出支持的长度
        ...
        {
            return File.Exists(path) && PathEx.FileIsUsed(path)
        ...
        {
            IFeatureClass featureClass = null; string message = null;
            try
            {
                featureClass = FeatureClassEx.FromPath(path, autoAddExtension);
                featureClass?.IsLock(out message);
                return message;
            }
            catch (Exception ex) { throw new Exception($"打开或检查要素类是否锁定失败：{ex.Message}\r\n请检查要素类路径“{path}”无误", ex); }
            finally { if (featureClass != null) Marshal.ReleaseComObject(featureClass); }
        }
        /// <summary>
