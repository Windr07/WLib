...
namespace WLib.ArcGis.DataCheck.Compare.Item
{
    ...
    public class CompareItemCollection : List<CompareItem>
    {
        ...
        public List<string> GetCompareFields(out List<string> leftFields, out List<string> rightFields)
        {
            ...
            fields = fields.Distinct().Where(v => !FieldCompare.SpecialKeywords.Contains(v.ToLower())).ToList();//移除重复字段、特殊字段、关键字、系统函数
            ...
            for (int i = 0; i < this.Count; i++)
            {
                switch (this[i])
                {
                    case FieldCompare fieldCompare:
                        lines[i] = $"左表字段【{fieldCompare.LeftColumnName}\t右表字段【{fieldCompare.LeftColumnName}】\t表达式【{fieldCompare.FieldExpression}】\t{fieldCompare.Description}";
                        break;
                    default:
