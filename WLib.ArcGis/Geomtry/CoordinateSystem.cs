...
namespace WLib.ArcGis.Geomtry
{
    ...
    public class CoordinateSystem
    {
        ...
        /// <returns></returns>
        public static ISpatialReference CreateSpatialReference(esriSRProjCS4Type pcsType)
        {
            return CreateSpatialReference((int)pcsType, ESrType.Projected);
        }
        /// <summary>
        /// 根据指定坐标系代码创建坐标系
        /// </summary>
        ...
        /// <param name="type">创建的坐标系类别</param>
        /// <returns></returns>
        public static ISpatialReference CreateSpatialReference(int factoryCode, ESrType type = ESrType.Projected)
        {
            ISpatialReferenceFactory spatialRefFac = new SpatialReferenceEnvironmentClass();
            ISpatialReference pcsSpatialRef = null;

            if (type == ESrType.Geographic)
                pcsSpatialRef = spatialRefFac.CreateGeographicCoordinateSystem(factoryCode);
            else if (type == ESrType.Projected)
                pcsSpatialRef = spatialRefFac.CreateProjectedCoordinateSystem(factoryCode);

            return pcsSpatialRef;
        }
        /// <summary>
        /// 创建空坐标系
        /// </summary>
        ...
        public static string GetCoordinateDetail(IFeatureClass featureClass)
        {
            ...
            return GetCoordinateDetail(((IGeoDataset)featureClass).SpatialReference);
        }
        /// <summary>
        /// 获取坐标系详细参数信息
        /// </summary>
        ...
        public static string GetCoordinateDetail(ISpatialReference spatialRef)
        {
            ...
            try
            {
                IPRJSpatialReferenceGEN gen1 = spatialRef as IPRJSpatialReferenceGEN;
                if (gen1 != null)
                {
                    gen1.ExportSpatialReferenceToPRJ(out str, out cBytes);
                }
                else
                {
                    IESRISpatialReferenceGEN gen2 = spatialRef as IESRISpatialReferenceGEN;
                    gen2.ExportToESRISpatialReference(out str, out cBytes);
                }
        ...
        public static bool CheckCoordinate(ISpatialReference spatialRef1, ISpatialReference spatialRef2, out string message)
        {
            ...
            if (result)
            {
                var str1 = GetCoordinateDetail(spatialRef1);
                var str2 = GetCoordinateDetail(spatialRef2);
        ...
        public static bool CheckMapCoordinate(IMap map, out string message)
        {
            ...
            if (result)
            {
                string str1 = GetCoordinateDetail(map.SpatialReference);
                string str2 = GetCoordinateDetail(featureClasses[0]);
                if (str1 != str2)
                {
